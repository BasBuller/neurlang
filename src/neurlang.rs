...
impl<F: Clone, T: ExecuteAST<F>> ASTNode<F, T> {
    ...
    pub fn execute(&self) -> T {
        match &self.op {
            ASTOp::Value { value } => value.value(),
            ASTOp::Unary { value, op } => {
                let value = value.execute();
                match op {
                    UnaryOp::Negate => value.negate(),
                    UnaryOp::Exponential => value.exp(),
                    UnaryOp::Log => value.log(),
                }
            }
            ASTOp::Binary {
                left_value,
                ...
                match op {
                    ...
                    BinaryOp::Sub => left_value.sub(&right_value),
                    BinaryOp::Mul => left_value.mul(&right_value),
                    BinaryOp::Div => left_value.div(&right_value),
                    BinaryOp::Pow => left_value.pow(&right_value),
                    BinaryOp::CompareEqual => left_value.compare_equal(&right_value),
            ...
            ASTOp::Reduce { value, dim, op } => {
                ...
                match op {
                    ReduceOp::Sum => array.reduce_sum(&value.shape, *dim),
                    ReduceOp::Max => array.reduce_max(&value.shape, *dim),
                }
            }
            ...
            ASTOp::Pad {
                value,
                axes_padding,
            } => value.execute().pad(&value.shape, (*axes_padding).clone()),
        }
    }
}
...
pub trait ExecuteAST<F: Clone> {
    ...
    fn pad(&self, shape: &Shape, axes_padding: Vec<PadAxis<F>>) -> Self;
